...
use crate::{EditableText, TextBrush, TextEdit};
...
pub struct CharacterMask {
    ...
    ///
    /// Defaults to `*`: the embedded default font is a subset that may not
    /// contain the typographic bullet, so `*` is the safe default. If the
    /// field's font covers U+2022, prefer `glyph: '\u{2022}'` (`•`) for
...
/// Inserts `text` into a masked field: the real chars go into the shadow
...
/// are transformed to keep the editor all-glyphs and the real value in the
/// shadow slot; cursor, selection, and point ops pass through untouched
/// (geometry over the mask string is the point of the design).
pub(crate) fn apply_masked_edit(
    ...
    match edit {
        ...
        TextEdit::ImeCommit { value: commit } => {
            ...
            clear.apply(driver, clipboard, max_characters, char_filter);
            masked_insert(
                commit.as_str(),
                mask,
                value,
...
mod tests {
    ...
    fn masked_field(initial: &str) -> (App, Entity) {
        ...
        // no-op -- register the embedded subset font explicitly
        ...
        editable_text.editor.set_text(initial);
    ...
    fn filter_applies_to_real_chars_not_glyphs() {
        ...
        // accept '5' into a masked field even though the glyph fails it
        let (mut app, entity) = masked_field("");
        app.world_mut()
            .entity_mut(entity)
